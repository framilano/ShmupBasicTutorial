...
function draw_ship_and_flame()
    if game.ship.invul <= 0 then
        ...
        if sin(game.frame_counter / 3) < 0 then
            for i=1,15 do
                pal(i, 7)
            end
            spr(game.ship.sprite, game.ship.x, game.ship.y)
...
function draw_muzzle()
    if game.ship.muzzle > 0 then
        circ(game.ship.x + 3, game.ship.y - 3, game.ship.muzzle, 7)
    end
end
...
function draw_enemies()
    for enemy in all(game.enemies) do
        ...
        if enemy.type == "green" then
            draw_entity_animation(enemy, green_enemy_living, true)
        end
        pal()
    end
end
...
function draw_explosions()
    for explosion in all(game.explosions) do
        if explosion.type == "normal" then
            animation_status = draw_entity_animation(explosion, normal_explosion)
            if (animation_status == "done") del (game.explosions, explosion)
        end
    end
end
...
function draw_entity_animation(entity_to_animate, animations_info, loop)
    ...
    for i = 1, #animations_info.sprites do
        while entity_to_animate.animation_timings[i] < animations_info.limits[i] do
            spr(
            ...
            entity_to_animate.animation_timings[i] += 1
    ...
    if should_loop then
        ...
        for i = 1, #entity_to_animate.animation_timings do
            entity_to_animate.animation_timings[i] = 0
        end
        ...
        entity_to_animate.animation_timings[1] += 1
    else
        return "done"
    end
end